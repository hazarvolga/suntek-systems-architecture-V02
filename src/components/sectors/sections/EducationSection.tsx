...
export default function EducationSection({ id }: { id: string }) {
    const stats = {
        metric: "50,000+",
        metricLabel: "Concurrent User Capacity",
        metricSub: "High Density Coverage",
        standard: "Wi-Fi 6E",
        standardLabel: "Connectivity Standard",
        standardSub: "Seamless Roaming",
        redundancy: "Mesh Grid",
        redundancyLabel: "Network Topology"
    };

    const schematic = {
        label: "Campus Topology: Zone 1 (Lecture Halls)",
        title: "Architectural Focus: High-Density Mesh",
        description: "Our adaptive mesh network handles massive concurrent connections during lecture hours, dynamically load-balancing traffic between APs to prevent bandwidth saturation.",
        hotspot: "High-Capacity AP Controller",
        hotspotDetail: "support up to 4k clients/AP | AI-driven RF Optimization"
    };

    const inventory = [
        {
            title: "Campus Core Router",
            code: "ST-EDU-CORE",
            icon: Router,
            specs: [
                "100GbE Uplink Capacity",
                "Deep Packet Inspection for Content Filtering",
                "Eduroam & Guest Network Isolation"
            ]
        },
        {
            title: "Lecture Capture Server",
            code: "ST-MEDIA-SVR",
            icon: Server,
            specs: [
                "Real-time 4K transcoding",
                "Automated cloud archiving",
                "Low-latency streaming to overflow rooms"
            ]
        },
        {
            title: "Smart Classroom Gateway",
            code: "ST-IOT-GW",
            icon: Wifi,
            specs: [
                "Protocol translation (Zigbee/Matter)",
    ...
    return (
        <section id={id} className="scroll-mt-24 py-16 border-b border-grid-line/50">
            <div className="flex items-center gap-4 mb-8">
                <span className="text-primary font-mono text-xs border border-primary/30 px-2 py-1">SEC_02</span>
                <div className="flex items-center gap-2">
                    <GraduationCap className="size-5 text-primary" />
                    <h2 className="text-3xl lg:text-4xl font-bold text-off-white tracking-tight">Education</h2>
                </div>
            </div>
