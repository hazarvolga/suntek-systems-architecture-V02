...
export default function HealthcareSection({ id }: { id: string }) {
    const stats = {
        metric: "%99.999",
        metricLabel: "Network Availability",
        metricSub: "SLA Guaranteed",
        standard: "AES-256",
        standardLabel: "Data Security Standard",
        standardSub: "HIPAA / GDPR Compliant",
        redundancy: "N+1 Tier III",
        redundancyLabel: "Redundancy Level"
    };

    const schematic = {
        label: "Live System Architecture: Clinical VLAN 402",
        title: "Architectural Focus: Low-Latency Isolation",
        description: "By implementing hardware-level micro-segmentation for medical imaging (PACS) systems, we ensure 100% diagnostic accessibility, unaffected by guest Wi-Fi or administrative traffic.",
        hotspot: "Core Switch Stack",
        hotspotDetail: "40Gbps Backbone | Low Latency Buffer 32MB"
    };

    const inventory = [
        {
            title: "Ultra-Dense Edge Computing",
            code: "ST-EDGE-900",
            icon: Router,
            specs: [
                "Max 10ms Jitter for VoIP devices",
                "Power-over-Ethernet (PoE++) support",
                "Reinforced fanless thermal cooling"
            ]
        },
        {
            ...
            icon: Lock,
            specs: [
                "End-to-end encrypted door signaling",
                "Offline cache for 50k+ identities",
                "Integrated with fire safety systems"
            ]
        },
        {
            title: "Redundant Storage Array",
            code: "ST-SAN-NVME",
            icon: Server,
            specs: [
                "All-Flash NVMe architecture",
                "Automatic cross-region snapshot",
                "Zero-trust file system encryption"
            ]
        }
    ];
    ...
    return (
        <section id={id} className="scroll-mt-24 py-16 border-b border-grid-line/50">
            <div className="flex items-center gap-4 mb-8">
                <span className="text-primary font-mono text-xs border border-primary/30 px-2 py-1">SEC_01</span>
                <div className="flex items-center gap-2">
                    <Activity className="size-5 text-primary" />
                    <h2 className="text-3xl lg:text-4xl font-bold text-off-white tracking-tight">Healthcare</h2>
                </div>
            </div>
